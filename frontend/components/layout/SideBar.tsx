    ...
    <>
      ...
      >
        <button
          ...
          className="absolute -right-3 top-12 z-100 hidden h-6 w-6 cursor-pointer items-center justify-center rounded-full border-2 border-slate-950 bg-blue-600 text-white shadow-lg transition-colors hover:bg-blue-500 lg:flex"
          ...
          <div className="flex items-center gap-3">
            ...
            {!isCollapsed && (
              <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col overflow-hidden">
                <span className="text-lg leading-none text-white uppercase" style={{ fontFamily: "var(--font-space-age)" }}>Boveda</span>
                <span className="mt-1 ml-0.5 text-[9px] tracking-[0.4em] text-blue-500 uppercase" style={{ fontFamily: "var(--font-space-age)" }}>Capital</span>
              </motion.div>
            )}
          </div>
        </div>
        ...
        <nav className="flex-1 space-y-2 overflow-y-auto px-4 py-2">
          {isLoading ? (
            ...
            <div className="mx-2 rounded-xl border border-red-500/20 bg-red-500/10 p-3 text-center">
              <p className="text-[10px] font-bold tracking-tighter text-red-400 uppercase">Falla de Enlace</p>
            </div>
          ) : (
            <>
              {!isCollapsed && (
                <p className="mb-4 px-4 text-[10px] font-bold tracking-[0.3em] text-slate-500 uppercase">Sistemas Activos</p>
              )}
                            ...
                            return (
                              <button
                                ...
                                className={cn(
                                  ...
                                  pathname === subPath ? "bg-blue-500/8 text-blue-300" : "text-slate-400 hover:text-slate-100"
                                )}
                              >
                                <DynamicIcon
                                  name={sub.icon}
                                  className={cn(
                                    "h-4 w-4 min-w-4",
                                    pathname === subPath ? "text-blue-400" : "text-slate-500"
                                  )}
                                />
                                {sub.name}
                              </button>
        ...
        <div className="mt-auto border-t border-slate-900/50 p-4">
          <div className={cn(
            "overflow-hidden rounded-[1.25rem] border border-slate-700/40 bg-slate-800/25 transition-all duration-300",
            isCollapsed ? "p-2" : "p-4"
          )}>
            <div className={cn("mb-4 flex items-center gap-3", isCollapsed && "mb-0 justify-center")}>
              ...
              {!isCollapsed && (
                <div className="flex flex-col overflow-hidden">
                  <span className="truncate text-[13px] leading-tight font-bold text-white">
